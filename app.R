...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      numericInput("x1", "Website Visitors", value = 200000),
      numericInput("x2", "Transactions", value = 12000),
      numericInput("x3", "Items/Transaction", value = 5),
      sliderInput("x4", "Customer Rating", min = 1, max = 10, value = 8.5),
      numericInput("x5", "Online Ads", value = 30000),
      actionButton("predictBtn", "Predict Sales")
    ),
    mainPanel(
      h3("Predicted Sales:",textOutput("predictedSales")),
...
server <- function(input, output) {
  observeEvent(input$predictBtn, {
    ...
    output$predictedSales <- renderText(paste0("$", round(predicted_sales, 2), "K"))
  })
